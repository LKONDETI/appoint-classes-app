---
name: new-feature
description: Full pipeline skill for building a new feature or API endpoint. Walks through all stages in order: plan → architect → implement → code review → security review → tests → run tests → summary. Invoke manually when starting a new feature.
disable-model-invocation: true
---

# New Feature Pipeline

You are orchestrating the full development pipeline for a new feature.

Ask the user: "What feature or endpoint do you want to build?" if not already specified.
...
Run the project's build and test commands to confirm everything is green.
